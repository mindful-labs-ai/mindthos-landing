...
export const metadata: Metadata = generatePageMetadata({
  title: '교육 프로그램',
  description:
    '상담사를 위한 마음토스의 교육 프로그램. 초심 상담사를 위한 사례개념화 웨비나, Next Genogram 심화 워크숍, 전문 상담가 2급 자격 취득 인턴십 안내.',
  path: '/education',
});
...
interface Program {
  ...
  cta: string;
  external?: boolean;
  /** true → 모집 마감. 상태 태그 muted + CTA 비활성 */
  closed?: boolean;
            ...
            {PROGRAMS.map((p, idx) => (
              <li key={p.title} className="edu-card">
                ...
                <div className="edu-card-body">
                  <span className={`t-tag edu-card-status${p.closed ? ' muted' : ''}`}>
                    {p.status}
                  </span>
