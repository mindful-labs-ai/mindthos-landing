...
export function BottomCTA({ ctaType, program }: BottomCTAProps) {
  ...
  return (
    <section className="mt-12 w-full rounded-2xl bg-gradient-to-br from-[var(--brand-primary)] to-[var(--brand-gradient-pair)] px-8 py-12 text-center">
      <p className="mb-2 text-[length:var(--t-small)] font-medium text-[var(--text-primary)] opacity-70">
        마음토스가 처음이신가요?
      </p>
      {cta.kicker && (
      ...
      <h2 className="mb-6 text-[length:var(--t-h2)] font-semibold text-[var(--text-primary)]">
        {cta.heading}
      </h2>
      <a
        href={cta.href}
        {...(cta.external ? { target: '_blank', rel: 'noopener noreferrer' } : {})}
        className="inline-block rounded-xl bg-[var(--bg-base)] px-8 py-4 text-[length:var(--t-lead)] font-semibold text-[var(--brand-primary-dark)] shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-200"
      >
        {cta.buttonText}
      </a>
    </section>
