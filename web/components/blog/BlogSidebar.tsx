  ...
  return (
    <aside className="hidden lg:flex flex-col gap-8 w-72 flex-shrink-0">
      ...
      <div className="rounded-xl border border-[var(--line-1)] bg-[var(--bg-base)] p-6">
        <h3 className="font-semibold text-[var(--text-heading-strong)] text-[length:var(--t-body)] mb-4">
          인기 글
        </h3>
        <ol className="flex flex-col gap-3">
          {popularPosts.length === 0 ? (
            <li className="text-[length:var(--t-small)] text-[var(--text-muted)]">
              아직 게시글이 없습니다.
            </li>
          ) : (
            popularPosts.slice(0, 5).map((post, i) => (
              <li key={post.id} className="flex items-start gap-3">
                ...
                <Link
                  href={`/blog/${post.slug}`}
                  className="flex-1 min-w-0 text-[length:var(--t-small)] text-[var(--text-body)] hover:text-[var(--brand-primary-dark)] transition-colors line-clamp-2"
                >
                  {post.title}
                </Link>
              </li>
          ...
          {categories.map((cat) => (
            <li key={cat.id}>
              <Link
                ...
                className="flex items-center justify-between px-3 py-2 rounded-lg text-[length:var(--t-small)] text-[var(--text-body)] hover:bg-[var(--bg-warm)] hover:text-[var(--brand-primary-dark)] transition-colors"
              >
                <span>{cat.name}</span>
      ...
      <div className="rounded-xl bg-[var(--bg-warm)] border border-[var(--line-warm)] p-6">
        <h3 className="font-semibold text-[var(--text-heading-strong)] text-[length:var(--t-body)] mb-1">
          뉴스레터
        </h3>
        <p className="text-[length:var(--t-small)] text-[var(--text-muted)] mb-3">
